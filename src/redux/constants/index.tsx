//contact
export const ALERT_SUCCESS = 'ALERT_SUCCESS';
export const ALERT_ERROR = 'ALERT_ERROR';
export const LOGIN = 'LOGIN';
export const LOGOUT = 'LOGOUT';
export const GET_USER_INFO = 'GET_USER_INFO';
export const GET_DMKHO = 'GET_DMKHO';
export const GET_DMNCC = 'GET_DMNCC';
export const GET_KHACHHANG = 'GET_KHACHHANG';
export const GET_SANPHAM = 'GET_SANPHAM';
export const GET_DONVITINH = 'GET_DONVITINH';
export const GET_LIST_TONKHO = 'GET_LIST_TONKHO';
export const GET_LIST_PHIEUNHAP = 'GET_LIST_PHIEUNHAP';
export const GET_DETAILPHIEUNHAP = 'GET_DETAILPHIEUNHAP';
export const GET_LIST_PHIEUXUAT = 'GET_LIST_PHIEUXUAT';
export const GET_DETAILPHIEUXUAT = 'GET_DETAILPHIEUXUAT';
export const GET_LIST_NHANSU = 'GET_LIST_NHANSU';

//Type contact
export type ALERT_SUCCESS = typeof ALERT_SUCCESS;
export type ALERT_ERROR = typeof ALERT_ERROR;
export type LOGIN = typeof LOGIN;
export type LOGOUT = typeof LOGOUT;
export type GET_USER_INFO = typeof GET_USER_INFO;
export type GET_DMKHO = typeof GET_DMKHO;
export type GET_DMNCC = typeof GET_DMNCC;
export type GET_KHACHHANG = typeof GET_KHACHHANG;
export type GET_SANPHAM = typeof GET_SANPHAM;
export type GET_DONVITINH = typeof GET_DONVITINH;
export type GET_LIST_TONKHO = typeof GET_LIST_TONKHO;
export type GET_LIST_PHIEUNHAP = typeof GET_LIST_PHIEUNHAP;
export type GET_DETAILPHIEUNHAP = typeof GET_DETAILPHIEUNHAP;
export type GET_LIST_PHIEUXUAT = typeof GET_LIST_PHIEUXUAT;
export type GET_DETAILPHIEUXUAT = typeof GET_DETAILPHIEUXUAT;
export type GET_LIST_NHANSU = typeof GET_LIST_NHANSU;
